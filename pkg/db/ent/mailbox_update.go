...
func (mbu *MailBoxUpdate) check() error {
	if v, ok := mbu.mutation.Content(); ok {
		if err := mailbox.ContentValidator(v); err != nil {
			return &ValidationError{Name: "content", err: fmt.Errorf(`ent: validator failed for field "MailBox.content": %w`, err)}
		}
	}
	return nil
}
...
func (mbuo *MailBoxUpdateOne) Save(ctx context.Context) (*MailBox, error) {
	...
	if len(mbuo.hooks) == 0 {
		...
		v, err := mut.Mutate(ctx, mbuo.mutation)
		if err != nil {
			return nil, err
		}
		nv, ok := v.(*MailBox)
...
// check runs all checks and user-defined validators on the builder.
func (mbuo *MailBoxUpdateOne) check() error {
	if v, ok := mbuo.mutation.Content(); ok {
		if err := mailbox.ContentValidator(v); err != nil {
			return &ValidationError{Name: "content", err: fmt.Errorf(`ent: validator failed for field "MailBox.content": %w`, err)}
		}
	}
