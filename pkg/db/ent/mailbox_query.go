...
func (mbq *MailBoxQuery) GroupBy(field string, fields ...string) *MailBoxGroupBy {
	grbuild := &MailBoxGroupBy{config: mbq.config}
...
func (mbq *MailBoxQuery) Select(fields ...string) *MailBoxSelect {
	...
	selbuild := &MailBoxSelect{MailBoxQuery: mbq}
	selbuild.label = mailbox.Label
	selbuild.flds, selbuild.scan = &mbq.fields, selbuild.Scan
	return selbuild
}
...
func (mbq *MailBoxQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*MailBox, error) {
	var (
		nodes = []*MailBox{}
		_spec = mbq.querySpec()
	)
